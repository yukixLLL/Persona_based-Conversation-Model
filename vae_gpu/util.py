...
def bow_loss_function(reals, bow_logit,batch_sz):
    ...
    return tf.nn.compute_average_loss(bow_loss, global_batch_size=batch_sz)
...
def loss_function(avg_rc_loss, avg_bow_loss, recog_mu, recog_logvar, prior_mu, prior_logvar, batch_sz,global_iter, kl_full_step):
    temp = 1 + (recog_logvar - prior_logvar) - tf.math.divide(tf.pow(prior_mu - recog_mu, 2),
                                                              tf.exp(prior_logvar)) - tf.math.divide(
                                                            tf.exp(recog_logvar), tf.exp(prior_logvar))
    kld = -1 / 2 * tf.reduce_sum(temp, axis=1)
    # avg_kld = tf.reduce_mean(kld)
    avg_kld = tf.nn.compute_average_loss(kld, global_batch_size=batch_sz)
